...
class Node :
    ...
    #hamming distance
    def getH(self) :
        return (self.getMisplacedTiles() + self.getManhattanDistance())
    
    #returns the misplaced tiles count
        ...
        for row in range(0,3) :
            for col in range(0,3) :
                ...
                if num != 0 :
                    ...
                    #indices of num in present state = row, col
                    #indices of num in goal state = goal_r , goal_c

                    #manhattan distance for num
                    x = abs(goal_r - row)
                    y = abs(goal_c - col)
                    #manhattan dist = x = y
                    ans = ans + x + y
